...
if [ -f "$SCRIPT_DIR/config/someip_wiper_client-proxy.json" ]; then
    VSOMEIP_CONFIGURATION="$SCRIPT_DIR/config/someip_wiper_client-proxy.json"
else
    GIT_ROOT=$(git rev-parse --show-toplevel)
    if [ -n "$GIT_ROOT" ] && [ -f "$GIT_ROOT/someip2val/config/someip_wiper_client-proxy.json" ]; then
        VSOMEIP_CONFIGURATION="$GIT_ROOT/someip2val/config/someip_wiper_client-proxy.json"
    fi
fi
