...
public:
    ...
    void on_get(const std::shared_ptr<vsomeip::message> &_message) {
            ...
            = vsomeip::runtime::get()->create_response(_message);
        {
            ...
            its_message << "* get() -> " << std::dec << payload_->get_length() << ") ";
            for (uint32_t i = 0; i < payload_->get_length(); ++i) {
                its_message << std::hex << std::setw(2) << std::setfill('0')
                    << (int) payload_->get_data()[i] << " ";
            }
    ...
    void on_set(const std::shared_ptr<vsomeip::message> &_message) {
        ...
        std::stringstream its_message;
        its_message << "* set (" << std::dec << payload_->get_length() << ") ";
        for (uint32_t i = 0; i < payload_->get_length(); ++i)
            its_message << std::hex << std::setw(2) << std::setfill('0')
                << (int) payload_->get_data()[i] << " ";
        std::cout << its_message.str() << std::endl;
    ...
    void run() {
        ...
        bool is_offer(true);
        bool is_offer_once = (::getenv("VSOMEIP_OFFER_ONCE") != NULL);
        while (running_) {
            if (is_offer)
                offer();
            else
